...
namespace Technie.PhysicsCreator.Rigid
{
	...
	public class RigidColliderCreatorWindow : EditorWindow, ICreatorWindow
	{
		...
		private static Collumn visibleCollumns = Collumn.Visibility | Collumn.Name | Collumn.Colour | Collumn.Type | Collumn.Material | Collumn.IsChild | Collumn.Trigger | Collumn.Paint | Collumn.Delete | Collumn.FaceThickness;
		...
		private void DrawHullGUI()
		{
			...
			if (areHullsFoldedOut)
			{
				...
				collumnWidths.Add(Collumn.FaceThickness,	IsCollumnVisible(Collumn.FaceThickness) ? 90 : 0);
				...
				bool putAddHullInline = IsCollumnVisible(Collumn.Name) || IsCollumnVisible(Collumn.Colour) || IsCollumnVisible(Collumn.Type) || IsCollumnVisible(Collumn.Material) || IsCollumnVisible(Collumn.Inflate) || IsCollumnVisible(Collumn.BoxFitMethod) || IsCollumnVisible(Collumn.MaxPlanes) || IsCollumnVisible(Collumn.IsChild) || IsCollumnVisible(Collumn.Trigger) || IsCollumnVisible(Collumn.FaceThickness);
				...
				GUILayout.BeginHorizontal();
				{
					...
					if (IsCollumnVisible(Collumn.FaceThickness))
						GUI.Label(GetCellRect(0, Collumn.FaceThickness), "Face thickness");
		...
		private void DrawHullGUILine(int hullIndex, Hull hull)
		{
			...
			GUILayout.BeginHorizontal ();
			{
				...
				if (IsCollumnVisible(Collumn.FaceThickness))
				{
					Rect baseRect = GetCellRect(row, Collumn.FaceThickness);

					// Keep a small label prefix so users can scrub the value by dragging it
					float prevWidth = EditorGUIUtility.labelWidth;
					EditorGUIUtility.labelWidth = 12f;

					float currValue = hull.faceThickness;
					float newValue = EditorGUI.FloatField(baseRect, new GUIContent("#"), currValue);

					if (currValue != newValue)
					{
						hull.faceThickness = newValue;
						EditorUtility.SetDirty(currentHullPainter.paintingData);
					}
		...
		private void DrawFaceDepth (float[] collumnWidths)
		{
			...
			GUILayout.BeginHorizontal ();
			{
				GUILayout.Label("Default face thickness", GUILayout.Width(collumnWidths[0]));

				currentHullPainter.paintingData.defaultFaceThickness = EditorGUILayout.FloatField(currentHullPainter.paintingData.defaultFaceThickness);

				float inc = 0.1f;
				if (GUILayout.Button("+", GUILayout.Width((collumnWidths[2]-4)/2)))
				{
					currentHullPainter.paintingData.defaultFaceThickness = currentHullPainter.paintingData.defaultFaceThickness + inc;
				}
				if (GUILayout.Button("-", GUILayout.Width((collumnWidths[2]-4)/2)))
				{
					currentHullPainter.paintingData.defaultFaceThickness = currentHullPainter.paintingData.defaultFaceThickness - inc;
		...
		private void DrawVisibilityToggles(float[] collumnWidths)
		{
			...
			DrawCollumnToggle(Collumn.FaceThickness, "Face Thickness", toggleWidth);
			GUILayout.EndHorizontal();
		}
		...
		private IEnumerator GenerateCollidersRoutine(RigidColliderCreator currentHullPainter, VhacdParameters parameters)
		{
			...
			{
				hull.GenerateCollisionMesh(sceneManipulator.GetTargetVertices(), sceneManipulator.GetTargetTriangles(), autoHulls);
			}

			// Sync the in-memory hull meshes with the asset meshes in hullAssetPath

			List<Mesh> existingMeshes = EditorUtils.GetAllMeshesInAsset (hullAssetPath);
