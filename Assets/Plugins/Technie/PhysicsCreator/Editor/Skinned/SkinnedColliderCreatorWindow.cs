		...
		private void CreateHullColliders()
		{
			...
			for (int i = 0; i < bones.Length; i++)
			{
				...
				foreach (BoneHullData data in hulls)
				{
					...
					// Create a new mesh if we don't have one
					if (data.hullMesh == null)
					{
						data.hullMesh = new Mesh();
					}

					// Clear any existing mesh data
					data.hullMesh.triangles = new int[0];
					data.hullMesh.vertices = new Vector3[0];
					
					CreateHullMesh(targetSkinnedRenderer, unpackedMesh, bone, data, CoordSpace.Bone, data.hullMesh);

					// Update the name
					data.hullMesh.name = data.Name + "_" + nextId;
					nextId++;

					if (data.hullMesh.triangles.Length > 0)
					{
						...
						{
							Vector3[] selectedVertices = data.hullMesh.vertices;
						...
						else if (data.colliderType == ColliderType.Capsule)
						{
							...
							CapsuleDef capDef = fitter.Fit(data.hullMesh.vertices, data.hullMesh.triangles);
						...
						else if (data.colliderType == ColliderType.Sphere)
						{
							...
							Sphere sphereDef = fitter.Fit(data.hullMesh.vertices, data.hullMesh.triangles);
...
#pragma warning restore 0162
		...
		private void SaveHullsToAsset()
		{
			...
			// Try and reuse/maintain meshes in the asset where possible, to avoid dropping references in other components when we regenerate the colliders
			...
			List<Mesh> existingMeshes = EditorUtils.GetAllMeshesInAsset(runtimeDataPath);
			
			// Remove any existing meshes which are now not referenced anywhere
			foreach (Mesh existing in existingMeshes)
			{
				if (!TargetEditorData.ContainsMesh(existing))
				{
					GameObject.DestroyImmediate(existing, true);
				}
			}

			// Save all the meshes in the runtime asset (if they're not already present in the asset)
			foreach (Mesh hull in hullMeshes)
			{
				Console.output.Log(string.Format("Adding {0} to asset at path {1}", hull, runtimeDataPath));
				if (!existingMeshes.Contains(hull))
					AssetDatabase.AddObjectToAsset(hull, runtimeDataPath);
			}
			EditorUtility.SetDirty(TargetEditorData.runtimeData);
		}

		public static void CreateHullMesh(SkinnedMeshRenderer skinnedRenderer, UnpackedMesh unpackedMesh, Transform bone, BoneHullData hull, CoordSpace outputSpace, Mesh destMesh)
		{
			...
			CalculateConvexHull(bone.name, hullPoints, hull.maxPlanes, destMesh);
		}
		...
		private static void CalculateConvexHull(string meshName, List<Point3d> hullPoints, int maxPlanes, Mesh destMesh)
		{
			...
			// Push data into the output mesh

			destMesh.name = meshName;
			destMesh.vertices = destVertices;
			destMesh.triangles = destIndices.ToArray();

			destMesh.RecalculateNormals();
			destMesh.RecalculateBounds();

			EditorUtility.SetDirty(destMesh);
		}
