                ...
                foreach (var t in allSceneObjs)
                {
                    ...
                    if (pr != null)
                    {
                        if (!prefabs.Contains(pr))
                        {
                            ...
                            if (optimizer)
                            {
                                OptimizerSceneData f = AllWithOptimizers.FirstOrDefault(ff => ff.prefabObject.FGetObjectIDHash() == pr.FGetObjectIDHash());
                                if (f == null) AllWithOptimizers.Add(new OptimizerSceneData(optimizer, pr, scenep));
                            }
                            else
                            {
                                OptimizerSceneData f = AllWithoutOptimizers.FirstOrDefault(ff => ff.prefabObject.FGetObjectIDHash() == pr.FGetObjectIDHash());
