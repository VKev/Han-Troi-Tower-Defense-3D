...
namespace TowerDefense3D.GridPlacement.Tests.EditMode
{
    public sealed class BoardSceneAuthoringTests
    {
        ...
        [Test]
        public void Planner_ProducesOneRectanglePerContiguousRoadRoleRun()
        {
            BoardDefinition board = CreateBoard(
                new GridDimensions(4, 1, 1),
                true,
                new[]
                {
                    ...
                    new BoardCellDefinition(new GridCell(2, 0, 0), BoardCellFlags.RoadSpawn),
                    new BoardCellDefinition(new GridCell(3, 0, 0), BoardCellFlags.RoadEnd),
                });

            BoardGeometryPlan plan = BoardGeometryPlanner.Create(board);

            Assert.That(plan.Rectangles, Has.Count.EqualTo(3));
            AssertRectangle(plan.Rectangles[0], BoardGeometryKind.RoadSurface, 0, 0, 0, 2, 1);
            AssertRectangle(plan.Rectangles[1], BoardGeometryKind.RoadSpawnSurface, 2, 0, 0, 1, 1);
            AssertRectangle(plan.Rectangles[2], BoardGeometryKind.RoadEndSurface, 3, 0, 0, 1, 1);
        }

        [Test]
        public void Planner_EmitsNoRoadRectanglesWhenNoRoadRoleCellsExist()
        {
            BoardDefinition board = CreateBoard(
                new GridDimensions(2, 2, 1),
                true,
                new[]
                {
                    new BoardCellDefinition(new GridCell(0, 0, 0), BoardCellFlags.SupportsPlacement),
                    new BoardCellDefinition(new GridCell(1, 1, 0), BoardCellFlags.StaticBlocker),
                });
            ...
            Assert.That(
                plan.Rectangles,
                Has.None.Matches<BoardGeometryRectangle>(rectangle =>
                    rectangle.Kind == BoardGeometryKind.RoadSurface
                    || rectangle.Kind == BoardGeometryKind.RoadSpawnSurface
                    || rectangle.Kind == BoardGeometryKind.RoadEndSurface));
        }

        [Test]
        public void Synchronizer_GeneratesColliderlessRoadOverlaysWithDistinctMaterialsAndNames()
        {
            BoardDefinition board = CreateBoard(
                new GridDimensions(4, 1, 1),
                true,
            ...
            Transform road = root.Find(BoardSceneSynchronizer.RoadAreaName);
            Transform spawn = root.Find(BoardSceneSynchronizer.RoadSpawnAreaName);
            Transform end = root.Find(BoardSceneSynchronizer.RoadEndAreaName);
            Assert.That(road, Is.Not.Null);
            Assert.That(spawn, Is.Not.Null);
            Assert.That(end, Is.Not.Null);

            foreach (Transform overlay in new[] { road, spawn, end })
            {
                Assert.That(overlay.GetComponent<MeshRenderer>(), Is.Not.Null);
                Assert.That(
                    overlay.GetComponents<Collider>(),
                    Is.Empty,
                    $"{overlay.name} must carry no collider.");
                Assert.That(overlay.GetComponent<MeshRenderer>().enabled, Is.True);
            }

            Assert.That(
                road.GetComponent<MeshRenderer>().sharedMaterial,
                Is.Not.EqualTo(spawn.GetComponent<MeshRenderer>().sharedMaterial));
            Assert.That(
                spawn.GetComponent<MeshRenderer>().sharedMaterial,
                Is.Not.EqualTo(end.GetComponent<MeshRenderer>().sharedMaterial));
            AssertGeneratedNamesAreReadable(root);
        }

        [Test]
        public void Synchronizer_RoadOverlaysFollowVisualizeInSceneToggle()
        {
            BoardDefinition board = CreateBoard(
                new GridDimensions(2, 1, 1),
                false,
                new[]
                {
                    new BoardCellDefinition(new GridCell(0, 0, 0), BoardCellFlags.Road),
            ...
            Transform root = presenterObject.transform.Find(BoardSceneSynchronizer.GeneratedRootName);
            Transform road = root.Find(BoardSceneSynchronizer.RoadAreaName);
            Assert.That(road, Is.Not.Null, "Overlay must still be generated even while hidden.");
            Assert.That(road.GetComponent<MeshRenderer>().enabled, Is.False);

            SetField(board, "visualizeInScene", true);
            BoardSceneSynchronizer.Synchronize(board);

            Transform roadAfterToggle = root.Find(BoardSceneSynchronizer.RoadAreaName);
            Assert.That(roadAfterToggle, Is.Not.Null);
            Assert.That(roadAfterToggle.GetComponent<MeshRenderer>().enabled, Is.True);
        }

        [Test]
        public void Synchronizer_OmitsRoadOverlaysWhenNoRoadRoleCellsExist()
        {
            BoardDefinition board = CreateBoard(
                new GridDimensions(2, 2, 1),
                true,
            ...
            Transform root = presenterObject.transform.Find(BoardSceneSynchronizer.GeneratedRootName);
            Assert.That(root, Is.Not.Null);
            Assert.That(root.Find(BoardSceneSynchronizer.RoadAreaName), Is.Null);
            Assert.That(root.Find(BoardSceneSynchronizer.RoadSpawnAreaName), Is.Null);
            Assert.That(root.Find(BoardSceneSynchronizer.RoadEndAreaName), Is.Null);
            Assert.That(root.childCount, Is.EqualTo(1));
        }

        [Test]
        public void Synchronizer_ReusesRoadOverlaysWithoutRegeneratingWhenBoardUnchanged()
        {
            BoardDefinition board = CreateBoard(
                new GridDimensions(2, 1, 1),
                true,
            ...
            Transform road = root.Find(BoardSceneSynchronizer.RoadAreaName);
            Assert.That(road, Is.Not.Null);
            int roadInstanceId = road.gameObject.GetInstanceID();
            int childCount = root.childCount;

            BoardSceneSynchronizer.Synchronize(board);

            Assert.That(root.childCount, Is.EqualTo(childCount));
            Transform roadAfterResync = root.Find(BoardSceneSynchronizer.RoadAreaName);
            Assert.That(roadAfterResync, Is.Not.Null);
            Assert.That(roadAfterResync.gameObject.GetInstanceID(), Is.EqualTo(roadInstanceId));
        }

        [Test]
        public void Synchronizer_RemovesRoadOverlaysWhenRoadCellsAreCleared()
        {
            BoardDefinition board = CreateBoard(
            ...
            BoardSceneSynchronizer.Synchronize(board);
            Transform root = presenterObject.transform.Find(BoardSceneSynchronizer.GeneratedRootName);
            Assert.That(root.Find(BoardSceneSynchronizer.RoadAreaName), Is.Not.Null);
