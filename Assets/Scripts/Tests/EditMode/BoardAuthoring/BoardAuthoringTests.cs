...
namespace TowerDefense3D.GridPlacement.Tests.EditMode
{
    public sealed class BoardAuthoringTests
    {
        private const string TemporaryFolder =
            "Assets/Scripts/Tests/EditMode/BoardAuthoring/__BoardPainterTemp";
        ...
        [Test]
        public void PainterOptions_ExposeApprovedBasicAndOverlayGroups()
        {
            Assert.That(
                BoardPainterWindow.BrushCategoryOptions,
                Is.EqualTo(new[] { "Basic Cell", "Overlay Cell" }));
            Assert.That(
                BoardPainterWindow.BasicCellPresetOptions,
                Is.EqualTo(new[]
                {
            ...
            Assert.That(
                BoardPainterWindow.OverlayCellOptions,
                Is.EqualTo(new[]
                {
                    "Prefab",
                    "Camera Focus",
        ...
        [Test]
        public void BasicCellPaint_ReplacesOnlyBasicFlagsAndPreservesOverlays()
        {
            ...
            Assert.That(
                document.GetFlags(coordinate),
                Is.EqualTo(BoardCellFlags.CameraFocus | BoardCellFlags.RoadEnd));
        }
        ...
        public void PaintEraseAndCommit_AffectOnlySelectedLevelAndSortYZX()
        {
            ...
            document.Paint(new GridCell(1, 2, 0), BoardPaintPreset.Buildable);
            document.Paint(new GridCell(0, 0, 1), BoardPaintPreset.NoBuild);
        ...
        [Test]
        public void PaintRoadBrush_UsesCenteredOddSizesAndClipsAtBoardEdgesForAllModes()
        {
            var dimensions = new GridDimensions(7, 7, 3);
            BoardDefinition definition = CreateTransientBoard(
                dimensions,
            ...
            Assert.That(
                BoardPainterWindow.PaintRoadBrush(
                    document,
                    new GridCell(3, 3, 0),
                    1,
                    RoadPaintMode.Road),
        ...
        [Test]
        public void PaintRoadBrush_SwitchingModeOnSameCells_KeepsExactlyOneRoadRoleBitPerCell()
        {
            var dimensions = new GridDimensions(5, 5, 1);
            BoardDefinition definition = CreateTransientBoard(
                dimensions,
                Array.Empty<BoardCellDefinition>());
            var document = new BoardAuthoringDocument(definition);
            var center = new GridCell(2, 2, 0);

            BoardPainterWindow.PaintRoadBrush(document, center, 3, RoadPaintMode.Road);
            BoardPainterWindow.PaintRoadBrush(document, center, 3, RoadPaintMode.End);
            ...
            for (int z = 1; z <= 3; z++)
            {
                ...
                {
                    Assert.That(
                        document.GetFlags(new GridCell(x, z, 0)),
                        Is.EqualTo(BoardCellFlags.RoadEnd));
                }
            }
        }

        [Test]
        public void RoadPaintModeUtility_GetRoadRole_ReturnsMatchingModeOrNone()
        {
            Assert.That(
                RoadPaintModeUtility.GetRoadRole(BoardCellFlags.None),
                Is.EqualTo(RoadPaintMode.None));
            Assert.That(
                RoadPaintModeUtility.GetRoadRole(
                    BoardCellFlags.SupportsPlacement | BoardCellFlags.Buildable),
                Is.EqualTo(RoadPaintMode.None));
            Assert.That(
                RoadPaintModeUtility.GetRoadRole(BoardCellFlags.Road),
                Is.EqualTo(RoadPaintMode.Road));
            Assert.That(
                RoadPaintModeUtility.GetRoadRole(
                    BoardCellFlags.RoadSpawn | BoardCellFlags.CameraFocus),
                Is.EqualTo(RoadPaintMode.Spawn));
            Assert.That(
                RoadPaintModeUtility.GetRoadRole(
                    BoardCellFlags.RoadEnd | BoardCellFlags.Buildable),
                Is.EqualTo(RoadPaintMode.End));
        }
        ...
        [Test]
        public void TemporaryAsset_SaveAndReloadPreservesLegacyStaticBlocker()
        {
            ...
            BoardDefinition definition = CreateTemporaryAsset(
                ...
                new[]
                {
                    new BoardCellDefinition(coordinate, BoardCellFlags.StaticBlocker)
                });
            string path = AssetDatabase.GetAssetPath(definition);
            AssetDatabase.SaveAssets();
            AssetDatabase.ImportAsset(path, ImportAssetOptions.ForceUpdate);
        ...
        {
            const string parent = "Assets/Scripts/Tests/EditMode/BoardAuthoring";
            if (!AssetDatabase.IsValidFolder(TemporaryFolder))
            {
