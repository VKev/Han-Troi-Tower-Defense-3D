...
namespace TowerDefense3D.GameFlow
{
    ...
    public sealed class LevelSceneContext : MonoBehaviour
    {
        ...
        public bool TryInitialize(LevelSceneRuntimeContext runtimeContext, out string error)
        {
            if (IsInitialized)
            {
                error = "Level scene context is already initialized.";
                return false;
            }

            if (!runtimeContext.IsValid)
            {
                error = "Level scene runtime context is invalid.";
                return false;
            }
        ...
        private void ShutdownInitializedParticipants()
        {
            ...
            {
                if (participants[index] is ILevelSceneParticipant participant)
                {
                    try
                    {
