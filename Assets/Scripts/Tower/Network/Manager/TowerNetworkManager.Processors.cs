...
namespace TowerDefense3D.Towers
{
    public sealed partial class TowerNetworkManager
    {
        ...
        private void StepProcessors()
        {
            for (int index = 0; index < orderedNodeIds.Count; index++)
            {
                ...
                if (!nodes.TryGetValue(nodeId, out NodeState node) ||
                    node.Spec.NetworkRole != TowerNetworkRole.Processor ||
        ...
        private static void DequeueProcessedInput(NodeState processor, int inputPort, ProjectileQueueEntry expectedInput)
        {
