  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (value === '--url') args.url = argv[++i];
      ...
      console.log(
        ...
        '  --state preparation|combat and --seed drive deterministic game hooks before capture\n' +
...
async function main() {
  ...
  if (args.state || args.seed !== undefined) {
    const applied = await page.evaluate(({ seed, state }) => {
      ...
      if (state === 'preparation' || state === 'combat') hooks.autoBuildMinimumChain?.();
      if (state === 'combat') {
        hooks.startWave?.();
        hooks.advance?.(6);
      } else if (state === 'soul-drag-preview') {
        hooks.setState?.('mastery-ready');
        hooks.startWave?.();
        // The onboarding hand intentionally waits for a real live target.
        hooks.advance?.(0.6);
  ...
  if (args.state === 'soul-drag-preview') {
    const button = await page.locator('#soul-skill').boundingBox();
    const target = await page.evaluate(() => window.__THREE_GAME_TEST_HOOKS__?.getSoulSkillTargetClientPoint?.() ?? null);
    if (button && target) {
