...
export type GamePhase = 'preparation' | 'wave' | 'paused' | 'won' | 'lost';
export type Branch =
...
export interface ProjectileState {
  readonly id: number; readonly payload: Payload; readonly group: THREE.Group; readonly trail: THREE.Line;
...
export interface SoulField {
  readonly id: number; readonly mesh: THREE.Group; readonly position: THREE.Vector3;
  readonly branch: 'base' | 'suppression' | 'conduction'; remaining: number; radius: number; tickTimer: number;
}
...
export const ELEMENT_NAMES: Record<Element, string> = {
  fire: 'Hỏa Hồn', ice: 'Băng Hồn', wind: 'Phong Hồn', earth: 'Địa Hồn',
};
export const BRANCH_NAMES: Record<Branch, string> = {
  rapid: 'Dồn Dập', heavy: 'Trọng Hồn', conduit: 'Dẫn Hồn', resonance: 'Cộng Hưởng',
  buff: 'Hộ Trợ', debuff: 'Nguyền Rủa', rapidPulse: 'Xung Nhanh', impactPulse: 'Xung Chấn',
  suppression: 'Trấn Hồn', conduction: 'Dẫn Truyền',
};
export const BRANCH_DESCRIPTIONS: Record<Branch, string> = {
  ...
  buff: 'Tăng tốc các linh ấn ở gần',
  ...
  suppression: 'Linh Vực làm chậm mạnh kẻ địch',
  conduction: 'Linh Vực tăng sát thương đường đạn',
};

export const NODE_DEFINITIONS: Record<NodeType, NodeDefinition> = {
  generator: {
    type: 'generator', name: 'Giếng Linh Hồn', shortName: 'Giếng Hồn', role: 'Sinh đạn', icon: '◈',
    description: 'Kết tinh linh hồn thành đạn Vật lý trung tính.', cost: 80, upgradeCost: 70,
    color: 0xe9d58a, interval: 0.92, capacity: Number.POSITIVE_INFINITY, connectionRange: 12.6, branches: ['rapid', 'heavy'],
  },
  fire: {
    type: 'fire', name: 'Ấn Hỏa Hồn', shortName: 'Hỏa', role: 'Biến đổi', icon: '◆',
  ...
  ice: {
    type: 'ice', name: 'Ấn Băng Hồn', shortName: 'Băng', role: 'Biến đổi', icon: '✦',
  ...
  wind: {
    type: 'wind', name: 'Ấn Phong Hồn', shortName: 'Phong', role: 'Biến đổi', icon: '➤',
  ...
  earth: {
    type: 'earth', name: 'Ấn Địa Hồn', shortName: 'Địa', role: 'Biến đổi', icon: '⬟',
  ...
  support: {
    type: 'support', name: 'Đài Hồn Phách', shortName: 'Hỗ Trợ', role: 'Duy trì', icon: '◉',
    description: 'Tích Linh Lực khi nhận đạn rồi chuyển tiếp nguyên trạng.', cost: 120, upgradeCost: 85,
    color: 0xb894ff, interval: 0.72, capacity: Number.POSITIVE_INFINITY, connectionRange: 8.1, branches: ['buff', 'debuff'],
  },
  special: {
    type: 'special', name: 'Chuông Vong Hồn', shortName: 'Xung Hồn', role: 'Bùng nổ', icon: '✹',
    description: 'Tích Xung Lực khi nhận đạn và nổ quanh chính nó.', cost: 180, upgradeCost: 110,
    color: 0xff9f65, interval: 0.68, capacity: 8, connectionRange: 12, branches: ['rapidPulse', 'impactPulse'],
  },
  nexus: {
    type: 'nexus', name: 'Tỏa Hồn', shortName: 'Tỏa Hồn', role: 'Kết thúc & giải phóng', icon: '⬢',
    description: 'Trụ duy nhất nhận tối đa hai chuỗi, tiêu thụ đạn và giải phóng Linh Vực.', cost: 0, upgradeCost: 120,
    color: 0x9d7cff, interval: 0, capacity: Number.POSITIVE_INFINITY, connectionRange: 0, branches: ['suppression', 'conduction'],
  },
};
...
export const ENEMY_DEFINITIONS: Record<EnemyKind, EnemyDefinition> = {
  swarm: { kind: 'swarm', name: 'Hồn Vụn', icon: '●', hp: 54, armor: 0, mr: 0, speed: 2.05, radius: 0.5, reward: 11, leakDamage: 1, layer: 0, color: 0xc965a7 },
  runner: { kind: 'runner', name: 'Ảnh Hồn', icon: '➤', hp: 72, armor: 5, mr: 5, speed: 3.35, radius: 0.48, reward: 15, leakDamage: 1, layer: 0, color: 0xec8e54, vulnerable: ['ice'] },
  armored: { kind: 'armored', name: 'Giáp Thi', icon: '⬟', hp: 220, armor: 60, mr: 10, speed: 1.18, radius: 0.78, reward: 34, leakDamage: 3, layer: 0, color: 0x7f6f77, resist: ['earth'], vulnerable: ['ice'] },
  wisp: { kind: 'wisp', name: 'Oán Hồn Bay', icon: '✦', hp: 92, armor: 0, mr: 12, speed: 2.25, radius: 0.54, reward: 20, leakDamage: 2, layer: 1, color: 0xe66b75, immune: ['fire'], vulnerable: ['ice'] },
  warded: { kind: 'warded', name: 'Hồn Hộ Mệnh', icon: '◇', hp: 420, armor: 10, mr: 60, speed: 1.05, radius: 0.92, reward: 75, leakDamage: 5, layer: 0, color: 0x8b78c8, reactionBarrier: 'steamBurst' },
  bulwark: { kind: 'bulwark', name: 'Vệ Hồn Hợp Kim', icon: '▣', hp: 520, armor: 65, mr: 18, speed: 1.08, radius: 0.98, reward: 72, leakDamage: 5, layer: 0, color: 0x5f7189, reactionBarrier: 'steamBurst', barrierDamageMultiplier: 0.04, speedAfterBarrierBreak: 1.85 },
  skyWarder: { kind: 'skyWarder', name: 'Hộ Vệ Thiên Hồn', icon: '✧', hp: 360, armor: 18, mr: 34, speed: 1.18, radius: 0.86, reward: 68, leakDamage: 4, layer: 1, color: 0xa888ff, resist: ['fire'], vulnerable: ['earth'], reactionBarrier: 'sandstorm', barrierDamageMultiplier: 0.08 },
  boss: { kind: 'boss', name: 'Chúa Vong', icon: '♛', hp: 980, armor: 35, mr: 35, speed: 0.72, radius: 1.18, reward: 165, leakDamage: 8, layer: 0, color: 0x514663, boss: true, resist: ['fire', 'earth'], vulnerable: ['ice', 'wind'], reactionBarrier: 'permafrost', barrierDamageMultiplier: 0.12 },
};
...
const TUTORIAL_WAVES: readonly WaveDefinition[] = [
  { title: 'Đọc Đường Đạn', clearReward: 55, healthMultiplier: 1, orders: sequence('swarm', 0.8, 4, 1.35) },
  { title: 'Tiếp Sức Hỏa', clearReward: 75, healthMultiplier: 1.08, orders: sequence('swarm', 0.6, 6, 1.08) },
  { title: 'Thử Thách Nhiệt', clearReward: 105, healthMultiplier: 1.18, orders: [...sequence('swarm', 0.5, 5, 1.02), ...sequence('runner', 2.2, 2, 1.85)].sort((a, b) => a.at - b.at) },
  { title: 'Tự Xây Linh Mạch', clearReward: 105, healthMultiplier: 6, orders: [...sequence('swarm', 0.4, 9, 0.85), ...sequence('runner', 1.1, 3, 1.25)].sort((a, b) => a.at - b.at) },
  { title: 'Áp Lực Song Hồn', clearReward: 125, healthMultiplier: 10, orders: [...sequence('swarm', 0.3, 10, 0.72), ...sequence('runner', 0.8, 5, 0.9), ...sequence('armored', 2.2, 1, 1)].sort((a, b) => a.at - b.at) },
  { title: 'Chứng Nhận Tỏa Hồn', clearReward: 150, healthMultiplier: 14, orders: [...sequence('swarm', 0.2, 12, 0.58), ...sequence('runner', 0.55, 7, 0.72), ...sequence('armored', 1.4, 2, 1.15)].sort((a, b) => a.at - b.at) },
];

const PRISMATIC_WAVES: readonly WaveDefinition[] = [
  { title: 'Đường Dài', clearReward: 50, healthMultiplier: 1.1, orders: sequence('swarm', 0.4, 10, 0.95) },
  {
    title: 'Dồn Ép Mặt Đất', clearReward: 70, healthMultiplier: 1.4,
    orders: [...sequence('runner', 0.3, 8, 0.82), ...sequence('swarm', 0.8, 6, 0.96)].sort((a, b) => a.at - b.at),
  },
  {
    title: 'Áp Lực Hai Tầng', clearReward: 100, healthMultiplier: 2,
    orders: [...sequence('swarm', 0.25, 10, 0.46), ...sequence('runner', 0.75, 8, 0.66), ...sequence('wisp', 0.55, 6, 0.72)].sort((a, b) => a.at - b.at),
  },
  {
    title: 'Chuông Vong Hồn', clearReward: 130, healthMultiplier: 3,
    orders: [...sequence('swarm', 0.15, 16, 0.34), ...sequence('runner', 0.4, 10, 0.44), ...sequence('wisp', 0.55, 8, 0.52)].sort((a, b) => a.at - b.at),
  },
  {
    title: 'Ma Trận Kháng Tính', clearReward: 170, healthMultiplier: 4.4,
    orders: [...sequence('runner', 0.12, 18, 0.29), ...sequence('armored', 0.45, 12, 0.43), ...sequence('wisp', 0.3, 16, 0.33)].sort((a, b) => a.at - b.at),
  },
  {
    title: 'Cuộc Vây Hãm Linh Giới', clearReward: 240, healthMultiplier: 6.2,
    orders: [
...
const CONVERGENCE_WAVES: readonly WaveDefinition[] = [
  {
    title: 'Biên Giới Mở Rộng', clearReward: 80, healthMultiplier: 1.3,
    orders: [...sequence('swarm', 0.2, 15, 0.56), ...sequence('runner', 0.9, 5, 0.92)].sort((a, b) => a.at - b.at),
  },
  {
    title: 'Xung Kích Mặt Đất', clearReward: 105, healthMultiplier: 1.7,
    orders: [...sequence('swarm', 0.15, 18, 0.44), ...sequence('runner', 0.55, 11, 0.62)].sort((a, b) => a.at - b.at),
  },
  {
    title: 'Gọng Kìm Hợp Kim', clearReward: 135, healthMultiplier: 2.4,
    orders: [
  ...
  {
    title: 'Lá Chắn Trên Mây', clearReward: 175, healthMultiplier: 3.3,
    orders: [
  ...
  {
    title: 'Hai Tầng Dồn Ép', clearReward: 215, healthMultiplier: 4.5,
    orders: [
  ...
  {
    title: 'Chúa Vong Đầu Tiên', clearReward: 270, healthMultiplier: 6,
    orders: [
  ...
  {
    title: 'Thiên Hồn Hợp Đội', clearReward: 325, healthMultiplier: 7.8,
    orders: [
  ...
  {
    title: 'Đội Chúa Vong', clearReward: 390, healthMultiplier: 10,
    orders: [
  ...
  {
    title: 'Ma Trận Phản Ứng', clearReward: 470, healthMultiplier: 12.5,
    orders: [
  ...
  {
    title: 'Đại Hội Tụ Vong Hồn', clearReward: 620, healthMultiplier: 15.5,
    orders: [
...
export const STAGES: readonly StageDefinition[] = [
  {
    title: 'Mạch Hồn Đầu Tiên', subtitle: 'Hướng dẫn mạng lưới mặt đất',
    path: [[-11, -2], [-3, -2], [-3, 2], [7, 2]],
    startingGold: 420, startingBaseHp: 3, tutorial: true, killRewardMultiplier: 1,
    board: { width: 10, depth: 7, originX: -9, originZ: -6, buildMaxX: 6.6, islandRadius: 15.8, cameraPosition: [16, 19, 18], cameraTarget: [-1, 1.4, -0.4] },
    waves: TUTORIAL_WAVES,
  },
  {
    title: 'Khe Nứt U Hồn', subtitle: 'Thử thách mạng hai tầng đường dài',
    path: [[-20, -6], [-4, -6], [-4, 5], [10, 5], [10, 0], [20, 0]],
    startingGold: 220, startingBaseHp: 20, tutorial: false, killRewardMultiplier: 1.5,
  ...
  {
    title: 'Đại Địa Vong Hồn', subtitle: 'Mạng hai tầng trên chiến trường mở rộng',
    path: [[-28, -9], [-15, -9], [-15, 7], [-5, 7], [-5, -5], [8, -5], [8, 9], [20, 9], [20, 1], [28, 1]],
    startingGold: 220, startingBaseHp: 20, tutorial: false, killRewardMultiplier: 1.65,
...
function highGroundPositions(valuesX: readonly number[], valuesZ: readonly number[]): readonly (readonly [number, number, number])[] {
  return valuesZ.flatMap((z) => valuesX.map((x) => [x, 3.05, z] as const));
}

export const HIGH_GROUND_PLATFORMS: readonly TerrainPlatformDefinition[] = ACTIVE_STAGE_INDEX === 1 ? [
  { center: [-8, 1.22, 1], size: [4.6, 2.44, 4.6], buildPositions: highGroundPositions([-9, -7], [0, 2]) },
  { center: [2, 1.22, 0], size: [8.6, 2.44, 6.6], buildPositions: highGroundPositions([-1, 1, 3, 5], [-2, 0, 2]) },
] : ACTIVE_STAGE_INDEX === 2 ? [
  { center: [-10, 1.22, -1], size: [8.6, 2.44, 10.6], buildPositions: highGroundPositions([-13, -11, -9, -7], [-5, -3, -1, 1, 3]) },
  { center: [-1, 1.22, -1], size: [6.6, 2.44, 10.6], buildPositions: highGroundPositions([-3, -1, 1], [-5, -3, -1, 1, 3]) },
  { center: [13, 1.22, 1], size: [6.6, 2.44, 10.6], buildPositions: highGroundPositions([11, 13, 15], [-3, -1, 1, 3, 5]) },
  { center: [5, 1.22, 1], size: [2.6, 2.44, 10.6], buildPositions: highGroundPositions([5], [-3, -1, 1, 3, 5]) },
] : [];
